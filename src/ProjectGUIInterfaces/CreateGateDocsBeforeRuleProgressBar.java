...
public class CreateGateDocsBeforeRuleProgressBar extends JPanel
    ...
    public CreateGateDocsBeforeRuleProgressBar(ConceptRelationsBuilder cons ) {
    ...
    public void actionPerformed(ActionEvent evt) {
        ...
        if(coB.getDocBeforRuleAnno().size()==0){
        coB.GateDocBuild(); 
        }
        ...
        task.execute();
         java.awt.EventQueue.invokeLater(new Runnable() {
                @Override
                public void run() {
                    new MeaningsBeforeAfterRuleAnno(coB.getMatchedConc(), coB.getDocBeforRuleAnno(),"معاني المعجم قبل الوسم بخرج القوالب").setVisible(true);
                }
            });
    }
    ...
    public static void createAndShowGUI(ConceptRelationsBuilder cpns) {
        //Create and set up the window.
        JFrame frame = new JFrame("بناء وثائق المعاني ووسمها بخرج المحللات");
        frame.setDefaultCloseOperation(JFrame.DISPOSE_ON_CLOSE);

        //Create and set up the content pane.
        JComponent newContentPane = new CreateGateDocsBeforeRuleProgressBar(cpns);
...
//        final CreateGateDocsBeforeRuleProgressBar pr=null;
